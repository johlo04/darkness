									...
									<?php foreach($user_groups as $g) { ?>
										<tr>
											...
											<td class="text-center">
												<a class="btn btn-icon-toggle btn-primary edit_group" href="<?php echo site_url('user_group/user_group/edit/' . $g['id']); ?>"><i class="fa fa-pencil"></i></a>
												<?php if($g['id'] > 2) { ?>
	...
	bootbox.dialog({
		...
		buttons: {
			success: {
				...
				callback: function() {
					$.ajax({
						url: "<?php echo site_url('user_group/user_group/remove_user_group'); ?>",
						data: {group_id: group_id},
						dataType: 'json',
						type: 'post',
						beforeSend: function() {
						...
						success: function(d) {
							loadUserGroup();
							makeToast(d.status, d.status_info);
						}, 
						error: function() {
							alert('Error Found!');
						}
