...
class Category extends MX_Controller {
	...
	public function index() {
		...
		$breadcrumbs = array(
			...
			'Category List'	=>	site_url('option/category'),	
		);
		...
		$config['base_url'] = site_url('option/category');
		...
		$filter['filter_grouping']= 'category'; //fixed
	...
	public function add(){
		...
		if($this->input->post() && $this->form_validation->run($this)==true){ //true means there no error
			...
			$post_category['grouping'] = 'category';
			
			$id = $this->Category_model->update($post_category, $this->input->post('id'));
			$this->session->set_flashdata('status_info', '<strong>Success! </strong> You have successfully added a Category');
			$this->session->set_flashdata('status', 'success');
			
			redirect(site_url('option/category/edit/'.$id));
		}else{
		...
		$data['system_name'] = 'Add Category';
		$data['page_title'] = 'Add Category Form';
		
		$data['header'] = modules::run('common/header/index', $data);
		$data['sidebar_left'] = modules::run('common/sidebar_left/index', $data);
		$data['sidebar_right'] = modules::run('common/sidebar_right/index', $data);
		$data['footer'] = modules::run('common/footer/index', $data);
		$data['form_link'] = site_url('option/category/add');
		
		//generate BrdCrmb
		$breadcrumbs = array(
			'home'	=>	site_url('option/category'),
			'Category list'	=>	site_url('option/category'),	
			'Add Category'	=>	site_url('option/category/add'),	
		);
	...
	public function edit(){
		...
		if($this->input->post() && $this->form_validation->run()==true){ //means there no error
			...
			$id = $this->Category_model->update($post_category, $this->input->post('id'));
			$this->session->set_flashdata('status_info', '<strong>Success! </strong> You have modified the Category detail .');
			$this->session->set_flashdata('status', 'success');
		...
		$data['system_name'] = 'Edit Category';
		$data['page_title'] = 'Edit Category Form';
		
		$data['header'] = modules::run('common/header/index', $data);
		$data['sidebar_left'] = modules::run('common/sidebar_left/index', $data);
		$data['sidebar_right'] = modules::run('common/sidebar_right/index', $data);
		$data['footer'] = modules::run('common/footer/index', $data);
		$data['form_link'] = site_url('option/category/edit/'.$this->uri->segment(4));
		
		$breadcrumbs = array(
			'home'	=>	site_url('option/category'),
			'Category list'	=>	site_url('option/category'),	
			'Edit Category'	=>	site_url('option/category/edit'),	
		);
	...
	function delete(){
		...
		if($id){
			...
			$this->session->set_flashdata('status_info', '<strong>Success! </strong> You have successfully deleted the Category.');
			$this->session->set_flashdata('status', 'success');
		}else{
			$this->session->set_flashdata('status_info', '<strong>Failed: </strong>Category may have been updated or no longer exist.');
			$this->session->set_flashdata('status', 'danger');
		}
		
		redirect(site_url('option/category'));
	}
